...
(define (top-level e)
  ...
  (define (T e)
    (match e
      ...
      [(? symbol? x) x]
      [`(,es ...) (map T es)]
      #;[`(,es ...) (define grd (gensym 'guard)) (define t (gensym 'true)) (define f (gensym 'false))
       `(letrec* ([,grd (procedure? (car ,es))] [,t ,(map T es)] [,f (raise "non-func-value")])
                    ,(T `(if ,grd ,t ,f)))]
      #;[`(,es ...)
       (pretty-print es)
       (pretty-print (car es))
       `(if (procedure? (eval (car es)))
                `,(map T es)
               (raise 'non-function-value-applied))]
      #;[`(,es ...)
       (T `(if (null? (car ,es))
               '()
               (if (procedure? (car ,es))
